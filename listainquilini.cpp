...
{
    //debug
    Inquilino *ranzy=new Admin("ranzy", "cuea");
    aggiungi(ranzy);
    Inquilino* fanculo= new Inquilino("fanculo", "fanculo");
    aggiungi (fanculo);
...
void ListaInquilini::creaInserisci(const std::string &user, const std::string &pw)
{
    if(checkPresenza(user))
        return;
    else{
        Inquilino *p=new Inquilino(user, pw);
        aggiungi(p);
    }
}
...
void ListaInquilini::checkAdmin(unsigned int pos)
{
        Inquilino* i= getInquilino(pos);
        if(i->isAdmin()==1)
        {
